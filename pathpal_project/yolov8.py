...
def setup_model(model_path, target='rk3566', device_id=None):
    ...
    Args:
        model_path: Path to model file (.pt, .rknn, or .onnx)
        target: Target RKNPU platform (for .rknn models)
        device_id: Device ID (for .rknn models)
    
    Returns:
        model: Model container object
        platform: Platform string ('pytorch', 'rknn', or 'onnx')
    """
    if model_path.endswith('.pt') or model_path.endswith('.torchscript'):
        ...
        raise ValueError("{} is not rknn/pytorch/onnx model".format(model_path))
    print('Model-{} is {} model, initialized successfully'.format(model_path, platform))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_folder', type=str, default='models/yolov8', help='img folder path')
    parser.add_argument('--coco_map_test', action='store_true', help='enable coco map test')

    args = parser.parse_args()

    # init model
    model, platform = setup_model(args.model_path, args.target, args.device_id)
    ...
    for i in range(len(img_list)):
        ...
        if args.img_show or args.img_save:
            ...
            if boxes is not None:
                draw(img_p, co_helper.get_real_box(boxes), scores, classes, verbose=True)
